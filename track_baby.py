...
def kidharGayaBe_baby(gray,tmp,rect,pprev, p_thresh):
...
def track_baby(input_path, output_path):
	...
	h, w, _ = img[0].shape
	vidWriter = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), 24, (w, h))
	count = 0
	...
	for im in images[0:]:
		...
		HGIM = cv2.equalizeHist(GIM)
		P = [np.zeros((6,1))]*3
		P, idx = kidharGayaBe_baby(HGIM,G,box,P, p_thresh)
...
def track_bolt(input_path, output_path):
	...
	for im in images[0:]:
		...
		GIM = cv2.cvtColor(IM,cv2.COLOR_BGR2GRAY)
		P = [np.zeros((6,1))]*3
		P, idx = kidharGayaBe_baby(GIM,G,box,P, p_thresh)
